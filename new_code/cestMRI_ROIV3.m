...
function   [CESTfit_ROI,MAPS] = cestMRI_ROIV3(cest_mri_prescan,cest_mri_postscan,ppm,mask,Method)
...
CESTfit_ROI.cfit.pars=beta;
CESTfit_ROI.cfit.residuals=R;

   
    [Lsum,L]=lorentzian(beta,ppmadj);
            CESTfit_ROI.Lsum=Lsum;
            CESTfit_ROI.cfitall=L;
            CESTfit_ROI.Zspectrum_prescan=Zspectrum_prescan;
            CESTfit_ROI.Zspectrum_postscan=Zspectrum_postscan;
            CESTfit_ROI.Difference=Difference;
            CESTfit_ROI.ppmadj=ppmadj';
%% 9.6) Calculate FWHM and CEST amplitudes
x = ppmadj;
y= CESTfit_ROI.cfitall(:,1);

% Julio on 08-Aug-2017

CESTfit_ROI.rsq = rsquare(CESTfit_ROI.Difference,CESTfit_ROI.Lsum);

z= CESTfit_ROI.cfitall(:,2);
maxCEST1 = max(y); 
maxCEST2 = max(z);
f = find(y==maxCEST1); 
cp = x(f);
...
CESTfit_ROI.fullwidth=Fullw;
CESTfit_ROI.CEST1amp=maxCEST1;
CESTfit_ROI.CEST2amp=maxCEST2;
end